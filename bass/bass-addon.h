...
typedef struct {
	...
	struct { // file functions
		...
		BOOL (WINAPI *StartThread)(BASSFILE file, DWORD bytespersec, DWORD offset); // finalize download buffer and start download thread
...
struct BASS_FUNCTIONS_FILE {
	...
	BOOL (WINAPI *Eof)(BASSFILE file); // End of file?
	const char *(WINAPI *GetTags)(BASSFILE file, DWORD tags); // get tags
	BOOL (WINAPI *StartThread)(BASSFILE file, DWORD bytespersec, DWORD minbuf, DWORD backbuf); // finalize download buffer and start download thread
	BOOL (WINAPI *CanResume)(BASSFILE file); // enough data buffered to resume?
};
